...
export default function Page() {
  ...
  const fetchImages = async () => {
    try {
      ...
      if (response.Contents) {
        ...
        setImages(imageItems.filter((item): item is ImageType => item !== null));
      }
    } catch (error) {
  ...
  return (
    <View style={CommonStyled.container}>
      <Text style={CommonStyled.titleText}>Upload image aws S3</Text>
      <Button title="Upload" onPress={pickImage} />
      <ScrollView style={CommonStyled.gallery}>
        {images &&
          images.map((img, index) => (
            <View key={index} style={CommonStyled.imageContainer}>
              <Image source={{ uri: img.uri }} style={CommonStyled.image} contentFit="cover" />
              <View style={CommonStyled.imageInfo}>
                <Text style={CommonStyled.infoText}>파일명: {img.fileName}</Text>
                <Text style={CommonStyled.infoText}>크기: {formatFileSize(img.fileSize)}</Text>
